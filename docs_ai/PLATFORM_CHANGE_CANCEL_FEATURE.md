# Platform Change Request Cancellation Feature

## Summary
Added the ability to cancel pending platform change requests. Only requests with 'pending' status can be cancelled.

## Changes Made

### 1. PlatformChangeRequest Model (`app/Models/PlatformChangeRequest.php`)
...
#### Migration File: `2025_12_01_102000_add_cancelled_status_to_platform_change_requests_table.php`
...
### 3. PlatformPartnerController (`app/Http/Controllers/Api/partner/PlatformPartnerController.php`)

#### New Endpoint: `cancelChangeRequest(Request $request)`

**Method**: POST  
**Route**: `/api/partner/platform/change/cancel`  
**Route Name**: `api_partner_platform_change_cancel`

**Request Parameters**:
- `change_request_id` (required, integer, exists in platform_change_requests)
...
- Logs info when change request is successfully cancelled (tracks change_request_id and platform_id)

### 4. API Routes (`routes/api.php`)

Added new route in the partner API group:
```php
Route::post('platform/change/cancel', [PlatformPartnerController::class, 'cancelChangeRequest'])
...
```bash
POST /api/partner/platform/change/cancel
...
The `platform_change_requests` table now has:
...
- When a change request is cancelled, the platform data remains unchanged (same as if the request was rejected)

## Related Features

This feature mirrors the cancellation functionality implemented for:
- **Platform Validation Requests** - Cancel validation requests
