...
inner join users u1 on ir.IdUser = u1.idUser inner join metta_users mu on ir.idUser = mu.idUser left join users u2 on ir.idUserResponse = u2.idUser where (ir.status = ? or ir.status = ?)", [StatusRequest::InProgressNational->value, StatusRequest::InProgressInternational->value]);
