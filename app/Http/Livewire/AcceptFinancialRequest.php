...
class AcceptFinancialRequest extends Component
{
    ...
    public function Confirmrequest($val, $num,$secCode, UserBalancesHelper $userBalancesHelper, settingsManager $settingsManager, BalancesManager $balancesManager)
    {
        ...
        {
            return redirect()->route('AcceptFinancialRequest', ['locale' => app()->getLocale(), 'numeroReq' => $num])->with('ErrorSecurityCodeRequest',Lang::get('Failed_Security_Code'));
        }
        $param = [
            'montant' => $financialRequest->amount,
            'recipient' => $financialRequest->idSender
        ];
        $soldeUser = $balancesManager->getBalances($userAuth->idUser);
        if ($val == 2) {
