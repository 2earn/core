CREATE PROCEDURE IF NOT EXISTS UpdateTotalAmountshares()
BEGIN
    DECLARE
done INT DEFAULT FALSE; DECLARE trans_id INT; DECLARE op_id INT; DECLARE trans_value DOUBLE; DECLARE trans_balance DOUBLE; DECLARE trans_user_id VARCHAR(9); DECLARE io_value CHAR(2); DECLARE last_balance DOUBLE;
...
SELECT
    ...
    beneficiary_id
FROM
    user_balances_shares
ORDER BY
    beneficiary_id,
    created_at,
    balance_operation_id ASC;
...
OPEN cur; read_loop: LOOP FETCH cur
INTO trans_id, op_id, trans_value, trans_user_id; IF done THEN LEAVE read_loop;
END IF;
...
FROM
    operations
WHERE
    idBalanceOperations = op_id;
-- Calculer le nouveau solde provisoire
IF io_value = 'I' THEN
SET
    last_balance = ROUND(last_balance + trans_value, 3); ELSEIF io_value = 'O' THEN
SET
    last_balance = ROUND(last_balance - trans_value, 3);
END IF;
