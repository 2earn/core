...
WHERE
    `u`.`idBalancesOperation` = 18 AND `u`.`value` > 0
GROUP BY CASE WHEN
    `u`.`idSource` = '11111111' THEN '2earn.cash' ELSE `u`.`idSource`
END,
IFNULL(
...
WHERE
    `u`.`idBalancesOperation` = 43 AND `s`.`is_representative` = 1
GROUP BY
    `c1`.`name`,
    IFNULL(
        CONCAT(
...
WHERE
    `u`.`idBalancesOperation` = 43 AND `u`.`idSource` <> '11111111' AND `s1`.`is_representative` = 1
GROUP BY
    IFNULL(
        CONCAT(
            IFNULL(
...
WHERE
    `u`.`idBalancesOperation` = 48 AND `s1`.`is_representative` <> 1
GROUP BY
    `c1`.`name`
UNION
SELECT
...
SELECT
    ...
    SUM(
        CASE WHEN `b`.`io` = 'I' THEN `u`.`value` ELSE - `u`.`value`
    END
) AS `weight`
FROM
    (
...
WHERE
    `u`.`idamount` = 1 AND `s1`.`is_representative` = 1
GROUP BY
    IFNULL(
        CONCAT(
            IFNULL(
...
WHERE
    `u`.`idBalancesOperation` = 51 AND `s`.`is_representative` <> 1
GROUP BY
    `c1`.`name`
UNION
SELECT
...
UNION
SELECT
    'SPONSORSHIP COMMISSION' AS `from`,
    `c`.`name` AS `to`,
    SUM(`u`.`value`) AS `weight`
FROM
    (
