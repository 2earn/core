<div
    ...
    <div class="d-flex align-items-start p-2">
        <div class="avatar-sm me-3 flex-shrink-0">
            <span class="avatar-title bg-warning bg-opacity-10 text-warning rounded-2 fs-18">
                <i class="las la-file-invoice-dollar"></i>
            </span>
        </div>
        <div class="flex-grow-1 overflow-hidden">
            <div class="d-flex justify-content-between align-items-start mb-1">
                <h6 class="fs-14 fw-semibold mb-1 text-dark">{{ __(__('notifications.settings.financial_request_sent')) }}</h6>
            ...
            <p class="text-muted mb-2 fs-13" style="line-height: 1.5;">{{ __('notifications.financial_request_sent.body', $notification->data['message_params'] ?? []) }}</p>
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <a href="{{ \App\Helpers\NotificationHelper::localizeUrl($notification->data['url'],app()->getLocale()) }}"
                   class="btn btn-sm btn-outline-primary px-3 py-1 d-inline-flex align-items-center gap-1">
                    <span class="fw-semibold" style="font-size: 0.8rem;">{{ __('notifications.financial_request_sent.action') }}</span>
