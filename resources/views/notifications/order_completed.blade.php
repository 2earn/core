<div
    ...
    <div class="d-flex align-items-start p-3 gap-3 position-relative"
         style="background: @if ($notification->read_at === null) linear-gradient(135deg, rgba(var(--bs-primary-rgb), 0.05) 0%, rgba(var(--bs-primary-rgb), 0.02) 100%) @else #ffffff @endif;">
        <div class="flex-grow-1 overflow-hidden">
            <div class="d-flex justify-content-between align-items-center mb-1 gap-1">
                <h6 class="fs-15 fw-bold mb-0 text-dark d-flex align-items-center gap-2">
                    {{ __('notifications.settings.order_completed') }}
                    @if ($notification->read_at === null)
            ...
            <p class="text-muted mb-1 fs-12" style="line-height: 1.2;">
                {{ __('notifications.order_completed.body', $notification->data['message_params'] ?? []) }}
            </p>
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <a href="{{ \App\Helpers\NotificationHelper::localizeUrl($notification->data['url'],app()->getLocale()) }}"
                   class="btn btn-sm btn-primary px-4 py-2 d-inline-flex align-items-center gap-2 rounded-pill"
                   style="transition: all 0.3s; box-shadow: 0 2px 6px rgba(var(--bs-primary-rgb), 0.25);"
                   onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(var(--bs-primary-rgb), 0.35)'"
                   onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 6px rgba(var(--bs-primary-rgb), 0.25)'">
                    <span class="fw-semibold">{{ __('notifications.order_completed.action') }}</span>
                ...
                <div class="d-flex align-items-center gap-2">
                    ...
                    @if ($notification->read_at === null)
                        <button type="button"
                                ...
                                onmouseover="this.style.background='rgba(var(--bs-primary-rgb), 0.1)'; this.style.color='var(--bs-primary)'"
