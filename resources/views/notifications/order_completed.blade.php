<div class="text-reset notification-item d-block dropdown-item position-relative" id="{{$notification->id}}"
    ...
    <div class="d-flex">
        ...
        <div class="flex-grow-1">
            ...
            <h5>{{ __(__('notifications.settings.order_completed')) }}</h5>
            <p>{{ __('notifications.order_completed.body', $notification->data['message_params'] ?? []) }}</p>
            <a href="{{ $notification->data['url'] }}" class="btn btn-sm btn-primary">
                {{ __('notifications.order_completed.action') }}
            </a>
