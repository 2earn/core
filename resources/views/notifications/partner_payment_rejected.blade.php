    ...
    <div class="d-flex align-items-start p-2">
        ...
        <div class="flex-grow-1 overflow-hidden">
            <div class="d-flex justify-content-between align-items-start mb-1">
                <h6 class="fs-14 fw-semibold mb-1 text-dark">{{ __('notifications.settings.partner_payment_rejected') }}</h6>
            ...
            <p class="text-muted mb-2 fs-13" style="line-height: 1.5;">
                {{ __('notifications.partner_payment_rejected.body', [
                    'amount' => $notification->data['message_params']['amount'] ?? '',
                    'reason' => $notification->data['message_params']['reason'] ?? ''
                ]) }}
            </p>
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                <a href="{{ \App\Helpers\NotificationHelper::localizeUrl($notification->data['url'],app()->getLocale()) }}"
                   class="btn btn-sm btn-outline-danger px-3 py-1 d-inline-flex align-items-center gap-1">
                    <span class="fw-semibold" style="font-size: 0.8rem;">{{ __('notifications.partner_payment_rejected.action') }}</span>
