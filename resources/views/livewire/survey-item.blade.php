<div class="card ribbon-box right border shadow-none overflow-hidden material-shadow">
    ...
    @if(!in_array( $currentRouteName,['surveys_results','surveys_participate']))
        <div class="card-footer bg-transparent">
            ...
            @if(intval($survey->status)>\Core\Enum\StatusSurvey::NEW->value)
                @if( ($survey->canShowResult() && $survey->enabled) ||strtoupper(auth()?->user()?->getRoleNames()->first())==\App\Models\User::SUPER_ADMIN_ROLE_NAME)
                        ...
                        <div class="row">
                          ...
                          {{ $survey->question->selection== \Core\Enum\Selection::MULTIPLE->value ? __('Multiple') : __('Unique')  }}                                      </span>
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-5">
                                ...
                                @if(strtoupper(auth()?->user()?->getRoleNames()->first())==\App\Models\User::SUPER_ADMIN_ROLE_NAME && intval($survey->status)==\Core\Enum\StatusSurvey::NEW->value)
                    ...
                    <div class="col-sm-12 col-md-12 col-lg-12">
                        <ul class="list-group mb-3">
                            @forelse ($survey->comments as $comment)
                                @if(strtoupper(auth()?->user()?->getRoleNames()->first())==\App\Models\User::SUPER_ADMIN_ROLE_NAME ||$comment->validated||$comment->user_id ==auth()->user()->id  )
                                    <li class="list-group-item mt-2">
                                        ...
                                        @if(!$comment->validated && strtoupper(auth()?->user()?->getRoleNames()->first())==\App\Models\User::SUPER_ADMIN_ROLE_NAME)
                                            <br>
