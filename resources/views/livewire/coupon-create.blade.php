            ...
            <form>
                <div class="row">
                    <div class="form-group col-md-4 mt-2">
                        ...
                        <input class="form-control" wire:model.live="attachment_date" type="date" id="attachment_date"
                            placeholder="{{__('Attachment Date')}}">
                        @error('attachment_date') <span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group col-md-4 mt-2">
                        <label for="value">{{__('value')}}:</label>
                        <input class="form-control" wire:model.live="value" type="number" id="value"
                            placeholder="{{__('value')}}">
                        @error('value') <span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group col-md-4 mt-2">
                        <label for="platform_id">{{__('Platform')}}</label>
                        <select class="form-select form-control @error('platform_id') is-invalid @enderror"
                            wire:model.live="platform_id" id="platform_id" aria-label="{{__('Platform')}}">
                    ...
                    <div class="form-group col-md-6 mt-2">
                        ...
                        <textarea class="form-control @error('pins') is-invalid @enderror" id="pins"
                            wire:model.live="pins" placeholder="{{__('Enter pins')}}"></textarea>
                        @error('description') <span class="text-danger">{{ $message }}</span>@enderror
                        <div class="form-text">{{__('Required field')}}</div>
                    </div>
                    <div class="form-group col-md-6 mt-2">
                        <label for="sn">{{__('SN')}}</label>
                        <textarea class="form-control @error('sn') is-invalid @enderror" id="sn" wire:model.live="sn"
