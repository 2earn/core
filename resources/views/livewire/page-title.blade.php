            ...
            <ol class="breadcrumb-modern m-0 flex-grow-1">
                ...
                @if($crumbs)
                    @foreach($crumbs as $index => $crumb)
                        ...
                        <li class="breadcrumb-item-modern @if($isLast) active @endif"
                            @if($isLast) aria-current="page" @endif>
                            <span class="breadcrumb-separator">/</span>
                            @if($crumb['isHtml'])
                                {{-- Item already contains HTML (like <a> tag), render as-is --}}
                                {!! $crumb['label'] !!}
                            @elseif(!$isLast && !empty($crumb['url']) && $crumb['url'] !== '#')
                                {{-- Create link from label and URL --}}
                                <a href="{{ $crumb['url'] }}" class="breadcrumb-link-modern">{!! $crumb['label'] !!}</a>
                            @else
                                {{-- Last item or no URL, render as text --}}
