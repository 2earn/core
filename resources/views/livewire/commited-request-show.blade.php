                    ...
                    @forelse($commitedInvestorsRequests as $commitedRequest)
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            {{$commitedRequest->request_date}} <span
                                    class="badge {{$commitedRequest->status==\Core\Enum\CommittedInvestorRequestStatus::Rejected->value?'bg-warning':'bg-success'}} ">{{__(\Core\Enum\CommittedInvestorRequestStatus::tryFrom($commitedRequest->status)->name)}}</span>
                        </li>
                    @empty
                    @endforelse
                </ul>
