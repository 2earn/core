...
var url = "/assets/json/";
var allmemberlist = ''; // var nurl ='C:\xampp\htdocs\modern\public\assets\json\team-member-list.json';

var nurl = "{{ URL::asset('assets/json/team-member-list.json') }}"; // Reading JSON with Fetch API

fetch('/members').then(function (res) {
  return res.json();
}).then(function (data) {
  allmemberlist = data;
...
var baseUrl = 'http://localhost:8000'; // load team data
...
function loadTeamData(datas) {
  ...
  Array.from(datas).forEach(function (teamData, index) {
    ...
    var isUserProfile = teamData.memberImg ? '<img src="http://localhost:8000/assets/images/logo-sm.png" alt="" class="member-img img-fluid d-block rounded-circle" />' : '<div class="avatar-title border bg-light text-primary rounded-circle text-uppercase">' + teamData.nickname + '</div>';
    document.querySelector("#team-member-list").innerHTML += '<div class="col">\
            <div class="card team-box">\
                <div class="team-cover">\
                    <img src="http://localhost:8000/assets/images/small/img-9.jpg" alt="" class="img-fluid" />\
                </div>\
